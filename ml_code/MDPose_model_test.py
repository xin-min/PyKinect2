...
from dataloader_MDPose import dataset_LSTM_changequat_singlemap, dataset_LSTM_changequat, dataset_changequat_withoriginalquat, dataset_LSTM, CustomDataset, CustomDataset_class, CustomDataset_window
...
train_dataloader = DataLoader(dataset_LSTM_changequat_singlemap(), batch_size= batch_size, shuffle=False)
...
def test(): 
	...
	f = open('punch1_testremovenoise_model.txt', 'w+') #### model
	f1 = open('punch1_testremovenoise_ref.txt', 'w+') #### ref
	f2 = open('punch1_testremovenoise_ground.txt', 'w+') #### ground
	...
	with torch.no_grad(): 
		...
		for i, data in enumerate(train_dataloader, 0):
			train_inputs, train_labels, train_original = data
			train_inputs = train_inputs.cuda()
			train_inputs = train_inputs[None, :]
			train_labels = train_labels.cuda()
			train_labels = train_labels[None, :]
			train_original = train_original.cuda()
			train_original = train_original[None, :]
			train_outputs = model(train_inputs)
			# print(str(train_outputs)+'\n')
			f.write(str(train_outputs)+'\n')
			f1.write(str(train_original)+'\n')
