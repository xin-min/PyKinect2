...
f = open('outputlog_10epochs_lstm.txt', 'a+')
PATH = "state_dict_model_10epochs_lstm.pt"
...
# train_dataloader = DataLoader(CustomDataset_class(), batch_size= batch_size, shuffle=True)
[train_dataloader, val_dataloader, test_dataloader] = random_split(CustomDataset_class(), [0.7, 0.2, 0.1], generator=torch.Generator().manual_seed(42))
...
class Network(nn.Module): 
	...
	def __init__(self):#, input_size, output_size):
		...
		self.cnn = nn.Sequential(
			...
			nn.Conv1d(512, 128, kernel_size=5, padding='same'),
			nn.BatchNorm1d(1), # batch norm
			nn.ReLU(),
...
for epoch in range(10):  # loop over the dataset multiple times
	...
	for i, data in enumerate(train_dataloader, 0):
		...
		if i % 5000 == 4999:    # print every 5000 mini-batches
			now = datetime.now()
			print(str(now) + f'[{epoch + 1}, {i + 1:5d}] loss: {running_loss / 5000:.3f}')
			f.write(str(now) + f'[{epoch + 1}, {i + 1:5d}] loss: {running_loss / 5000:.3f}'+'\n')
			running_loss = 0.0
